...
2026-07-06 Step 2.29: scoped runtime HTTP smoke CLI 已实现；新增 `runtime:http-smoke`，用 disposable `NNZ_POSTGRES_SCOPED_RUNTIME_URL` 启动真实 demo server 并验证 `/api/me/*` register/persona/chat/history/Covenant/export/delete HTTP 链路；清空 child `NNZ_LLM_*` 保持本地 smoke 确定性；本地 typecheck、165 tests + 2 skipped、build:demo、CLI help 通过
2026-07-06 Step 2.30: scoped runtime smoke suite 已实现；新增 `runtime:smoke-suite`，合并 direct runtime adapter smoke、`build:demo`、HTTP `/api/me/*` smoke；本地 typecheck、175 tests + 2 skipped、build:demo、CLI help 通过
...
2026-07-07 Step 2.35: release validation evidence option 已实现并推送；`release:validation-suite` 新增可选 `--evidence-out`，确认执行后成功或 stage 失败都会写脱敏 evidence JSON；本地 typecheck、206 tests + 2 skipped、build:demo、CLI help、当前环境 preflight-blocked evidence smoke、git diff --check 通过
2026-07-07 Step 2.36: sensitive local release safety 已实现并推送；`.gitignore` 显式忽略 SQLite/DB、raw/report/summary/evidence JSON、migration/release/snapshot artifact 目录，新增 `nnz-mvp/.env.example` 空模板和 disposable DB 安全注释；check-ignore 验证不影响 package/tsconfig/Markdown/env template；本地 typecheck、206 tests + 2 skipped、build:demo、git diff --check 通过
2026-07-07 Step 2.37: H5 graduation export + safety support UX 已实现并推送；`public/index.html` 的毕业流先导出用户数据档案再提交毕业，高风险 guard 回复出现时显示现实支持提示和热线拨号入口，状态徽标不再兜底显示内部状态名；新增 `src/h5-experience.test.ts` 固化回归；本地 h5 targeted tests、typecheck、全量 tests、build:demo、git diff --check 通过
2026-07-07 Step 2.38: H5 onboarding consent UX 已实现并推送；`public/index.html` 创建记忆伙伴前新增边界与数据权利确认，`POST /api/me/persona` 同步要求 `consentAccepted: true`，未确认时阻止创建，新建另一位时重置确认；`src/h5-experience.test.ts` / `src/demo-server-consent.test.ts` 增加回归；本地 targeted tests、typecheck、211 tests + 2 skipped、build:demo、git diff --check 通过；最新已推送提交 `59e8119 feat: add h5 onboarding consent gate`
2026-07-07 Step 2.39: H5 account deletion inline confirmation 已实现并推送；登录态“删除全部数据”不再使用浏览器 prompt，而是在页面内展开删除说明、建议先导出数据档案、输入“删除”后二次确认，再调用既有 `/api/me/delete`；新增 H5 静态回归测试；本地 `npm test -- h5-experience`、typecheck、212 tests + 2 skipped、build:demo、git diff --check 通过；最新已推送提交 `871c4d0 feat: add h5 delete confirmation`
2026-07-07 Step 2.40: H5 memory append UX/API 已实现并推送；新增 `/api/me/memory`，通过当前登录用户 + selected persona 的 scoped runtime 写入补充记忆；H5 对话区新增“补充记忆”面板，限定为已经发生过的细节并提醒只留下愿意保留的内容；`runtime:http-smoke` 覆盖新接口并断言导出包含补充记忆；与 Step 2.41 合并推送为 `adac0ea feat: add h5 memory append and graduation confirmation`
...
2026-07-08 Step 2.42: scoped runtime daily usage persistence 本地已实现；`ScopedPersonaRuntimeAdapter` / InMemory repository / Postgres scoped repository 新增 `updateRuntimeUsage()`，H5 `/api/me/chat` 的 scoped runtime guard 在 `checkDailyLimit()` 通过并 `incrementDailyCount()` 后显式写回 `dailyMessageCount` / `lastMessageDate`；更新保留 Covenant state、snapshot 和 NODE context；本地 targeted tests、typecheck、218 tests + 2 skipped、build:demo、git diff --check 通过，尚待下一次合并 push
2026-07-08 Step 2.43: H5 seal inline confirmation 本地已实现；ACTIVE 状态下“封存”不再一键提交，而是打开页面内确认面板，输入“安放”后才调用 `/api/me/seal`；切换/新建 persona 和封存成功会收起确认面板；本地 h5 targeted test、typecheck、219 tests + 2 skipped、build:demo、git diff --check 通过，建议与 Step 2.42 合并 push
```
...
2026-07-01 Step 2 migration readiness roadmap：新增 `nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md`，当时明确剩余 5 个目标、完成标准、推荐顺序和安全边界；Step 2.25 后仍剩 4 个未完全收口目标，但 scoped runtime 本地烟测入口已具备。当前无需每个小步骤都 push；应按目标连续推进，遇到真实 snapshot、disposable DB URL、云端 token 等外部输入点再做明确 checkpoint。
...
2026-07-03 Step 2.25 scoped runtime smoke guard 结果：新增 `src/postgres-env-alias-guard.ts`、`src/tools/postgres-scoped-runtime-smoke-cli.ts`、`src/tools/postgres-scoped-runtime-smoke-cli.test.ts`，新增 `runtime:smoke` script；`runtime-persistence-config` 现在会拒绝 `NNZ_POSTGRES_SCOPED_RUNTIME_URL` 与 `DATABASE_URL` / `NNZ_POSTGRES_URL` 同值，错误只含 env key；`runtime:smoke` 必须传 `--database-url-env NNZ_POSTGRES_SCOPED_RUNTIME_URL` 与 `--confirm RUN_POSTGRES_SCOPED_RUNTIME_SMOKE`，并验证 scoped runtime adapter 的 credential/persona/runtime context/Covenant/cross-scope/cascade/cleanup。stdout/失败输出不含 DB URL、fixture memory/chat、credential hash、row payload 或 raw DB error details。本地 `npm run typecheck`、targeted guard/runtime smoke tests、`npm run runtime:smoke -- --help`、`npm test`、`npm run build:demo` 通过；全量为 25 个测试文件、151 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-03-Step2.25-ScopedRuntimeSmokeGuard.md`。
...
2026-07-06 Step 2.29 scoped runtime HTTP smoke CLI 结果：新增 `src/tools/postgres-scoped-runtime-http-smoke-cli.ts` / `.test.ts`，新增 `runtime:http-smoke` script；命令必须传 `--database-url-env NNZ_POSTGRES_SCOPED_RUNTIME_URL` 与 `--confirm RUN_POSTGRES_SCOPED_RUNTIME_HTTP_SMOKE`，拒绝 `DATABASE_URL` / `NNZ_POSTGRES_URL` 与生产别名同值。它会在 scoped mode 下启动真实 `dist-cjs/demo-server.js`，清空 child `NNZ_LLM_*`，验证 `/healthz` scoped Postgres 诊断，再跑 `/api/register`、persona 创建、chat/history、Covenant seal/node/complete/graduate、export/delete，并通过 `/api/me/delete` 尝试清理 fixture。stdout/失败输出不含 DB URL、token、email/password、memory/chat、credential hash、row payload、server log 或 raw error details。本地 `npm run typecheck`、targeted HTTP smoke CLI tests、`npm run runtime:http-smoke -- --help`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 27 个测试文件、165 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-06-Step2.29-ScopedRuntimeHttpSmoke.md`。

2026-07-06 Step 2.30 scoped runtime smoke suite 结果：新增 `src/tools/postgres-scoped-runtime-smoke-suite-cli.ts` / `.test.ts`，新增 `runtime:smoke-suite` script；命令必须传 `--database-url-env NNZ_POSTGRES_SCOPED_RUNTIME_URL` 与 `--confirm RUN_POSTGRES_SCOPED_RUNTIME_SMOKE_SUITE`，拒绝 `DATABASE_URL` / `NNZ_POSTGRES_URL` 与生产别名同值。它会串行运行 direct `runtime:smoke`、`build:demo`、HTTP `runtime:http-smoke`，并支持仅在 build output 已确认当前时使用 `--skip-build`。suite stdout 只输出固定 stage，失败时不拼接子命令 stdout/stderr，避免 raw DB error、server log、child process output 或 secret 被带出。本地 `npm run typecheck`、targeted suite CLI tests、`npm run runtime:smoke-suite -- --help`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 28 个测试文件、175 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-06-Step2.30-ScopedRuntimeSmokeSuite.md`。

2026-07-06 Step 2.31 migration validation suite 结果：新增 `src/tools/postgres-scoped-migration-validation-suite-cli.ts` / `.test.ts`，新增 `migration:validation-suite` script；命令必须传显式 `--from-json` 或 `--from-sqlite`、三类输出路径、`--database-url-env NNZ_POSTGRES_INTEGRATION_URL` 与 `--confirm RUN_POSTGRES_SCOPED_MIGRATION_VALIDATION_SUITE`。suite 先跑 offline `migration:readiness`，readiness exit 0 后才跑 disposable `migration:smoke`；失败时不拼接子命令 stdout/stderr，避免 raw snapshot、raw DB error、child command output 或 secret 被带出。本地 `npm run typecheck`、targeted validation suite CLI tests、`npm run migration:validation-suite -- --help`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 29 个测试文件、183 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-06-Step2.31-MigrationValidationSuite.md`。
...
2026-07-07 Step 2.36 sensitive local release safety 结果：按产品进程审计指出的本地导出物误提交风险，更新根 `.gitignore`，显式忽略 SQLite/DB 文件、`raw.json` / `report.json` / `summary.json` / `evidence.json`、`*snapshot*.json`、migration/readiness report/summary JSON、release/validation evidence JSON，以及 `migration-artifacts/`、`release-artifacts/`、`snapshot-exports/`；新增 `nnz-mvp/.env.example`，只包含空 env key 和安全注释，标明 release validation 的 disposable DB 不能等于 production aliases。`git check-ignore` 已验证这些规则会命中典型本地导出物，且不会忽略 `package.json`、`package-lock.json`、tsconfig、Markdown 交接记录或 `.env.example`。本地 `npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 32 个测试文件、206 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-07-Step2.36-SensitiveArtifactIgnore.md`。

2026-07-07 Step 2.37 H5 graduation export + safety support UX 结果：按“产品帮助用户离开而不是留下”“数据主权”和安全告知红线，补齐 H5 毕业与高风险提示路径。`public/index.html` 现在把导出逻辑抽成 `h5DownloadDataArchive()`，用户点击“毕业”时会先拉取 `/api/me/export` 并下载数据档案，再提交 `/api/me/graduate`；毕业确认和完成状态都明确提示数据已交还给用户。对话区新增隐藏的 `h5SafetySupport` 支持提示条；当助手回复包含现有 guard 的心理援助热线或“请现在就联系能真正帮助你的人”时，H5 会显示现实支持提醒和 `tel:4001619995` 拨号入口。状态徽标文案改为用户可理解的“可继续说话 / 正在休息 / 特别时刻 / 已好好告别”，异常状态兜底显示“状态更新中”，不再把内部状态名原样显示；封存/特别时刻文案也弱化机制感，并对 covenant-state 查询的 persona id 做 URL encode。新增 `src/h5-experience.test.ts`，固定“导出发生在毕业提交前”“不显示 raw lifecycle state fallback”和“风险回复出现时有独立支持提示”三个回归点。本地 `npm test -- h5-experience`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 33 个测试文件、209 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-07-Step2.37-H5GraduationExport.md`。

2026-07-07 Step 2.38 H5 onboarding consent UX 结果：按产品进程审计指出的 onboarding / consent 缺口，给 H5 创建体验最后一步增加“辅助性记忆对话、不替代身边的人或专业帮助、可随时导出或删除自己的数据”的确认勾选。`h5CreatePersona()` 会在未确认时阻止创建、提示“请先确认使用边界和数据权利。”、回到第 3 步并聚焦确认框；提交 `/api/me/persona` 时显式带 `consentAccepted: true`；`h5ShowCreatePanel()` 新建另一位时会重置确认，避免上一位的确认状态复用。`src/demo-server.ts` 同步要求 `POST /api/me/persona` 的 body 带 `consentAccepted: true`，否则返回 400；`runtime:http-smoke` 已补同字段，避免未来 scoped runtime HTTP 验收绕过确认。新增 `src/h5-experience.test.ts` 与 `src/demo-server-consent.test.ts` 回归点，固定确认文案、创建前校验、新建重置行为和 API 层校验。本地 targeted tests、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、211 tests，另有 2 个 integration 文件 skipped。重启本地 demo server 后，localhost API smoke 验证缺 `consentAccepted` 创建返回 400，带 `consentAccepted:true` 创建返回 200。记录见 `nnz-mvp-2026-07-07-Step2.38-H5OnboardingConsent.md`。

2026-07-07 Step 2.39 H5 account deletion inline confirmation 结果：按产品进程审计里“Export / delete 的用户自助闭环还不完整”的缺口，继续补 H5 删除体验。登录态“删除全部数据”现在展开页面内确认面板，明确说明会清除当前账号下全部记忆、对话和创建内容且不可恢复，并建议先导出数据档案；用户输入“删除”后才会提交既有 `/api/me/delete`，后端确认常量仍只作为请求体发送，不作为用户主要体验文案。新增 `src/h5-experience.test.ts` 回归点，固定删除确认面板、不再使用 `prompt()`、错误确认会聚焦输入框。本地 `npm test -- h5-experience`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、212 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-07-Step2.39-H5DeleteInlineConfirm.md`。

2026-07-07 Step 2.40 H5 memory append UX/API 结果：按产品进程审计里 onboarding / memory collection 的前台缺口，给 H5 已创建 persona 增加“补充记忆”入口。`src/demo-server.ts` 新增 `POST /api/me/memory`，要求当前登录用户、`personaId` 和非空内容，先通过 `requireUserPersonaRuntime(res, authUser.userId, body.personaId)` 绑定当前用户的 scoped runtime，再写入 `DESCRIPTION` memory，避免 persona-only 访问。H5 对话区新增“补充记忆”按钮和面板，文案限定为“已经发生过的细节”，提醒“只补充你愿意留下的内容”，保存成功后更新“对话 / 记忆”计数。`runtime:http-smoke` 也补跑 `/api/me/memory` 并断言导出包含补充记忆。新增 H5/API/smoke 回归点。本地 `npm test -- h5-experience demo-server-consent postgres-scoped-runtime-http-smoke-cli`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、214 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-07-Step2.40-H5MemoryAppend.md`。

2026-07-07 Step 2.41 H5 graduation inline confirmation 结果：继续补齐 H5 graduation 旅程，把浏览器 `confirm()` 改成页面内确认面板。用户点击“毕业”后会看到“先导出数据档案、随后这段对话安静封存”的说明，输入“告别”后才执行；`h5Graduate()` 保持原有顺序：先 `GET /api/me/export` 并下载档案，再 `POST /api/me/graduate`，完成后收起确认面板并刷新状态。新增 H5 回归点，固定不再使用 `confirm(`、毕业按钮打开内联确认、确认词为“告别”、导出仍发生在毕业提交前。本地 `npm test -- h5-experience`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、215 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-07-Step2.41-H5GraduationInlineConfirm.md`。

2026-07-08 Step 2.42 scoped runtime daily usage persistence 结果：继续补“每日健康使用上限”在 scoped runtime / Postgres mode 下的真实落库路径。`ScopedPersonaRuntimeAdapter` 新增 `updateRuntimeUsage({ dailyMessageCount, lastMessageDate })`，InMemory scoped repository 更新当前 scope 的 session usage 字段；`PostgresScopedSoulRepository` 新增同名方法，读取当前 session 后只替换 usage 字段并通过既有 `setSession()` upsert 回 `nnz_runtime_sessions`，避免丢失 Covenant state、snapshot 或 NODE context。`applyUserRuntimeSafetyGuard()` 现在在 `checkDailyLimit()` 通过、`incrementDailyCount()` 后显式调用 `runtime.updateRuntimeUsage()`，让 H5 `/api/me/chat` 的 scoped runtime 路径不再只更新 session 副本。新增 adapter、Postgres repository 和 demo-server 静态回归点。本地 `npm test -- scoped-runtime-adapter postgres-scoped-soul-repository demo-server-consent`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、218 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-08-Step2.42-ScopedRuntimeDailyUsage.md`。

2026-07-08 Step 2.43 H5 seal inline confirmation 结果：继续补 Seal / Node / Graduation 用户旅程，把首次“封存”从一键立即执行改为页面内确认。`public/index.html` 新增 `h5SealConfirmPanel`，ACTIVE 状态下封存按钮改为 `h5OpenSealConfirm()`，用户输入“安放”后 `h5ConfirmSeal()` 才调用 `h5SealSoul()`；切换 persona、新建 persona、封存成功后会收起确认面板。`h5CovenantAction()` 返回 success boolean，让封存成功后再关闭确认面板。新增 H5 回归点，固定封存确认面板、确认词、“封存”不直接调用 API，以及 API 成功后才关闭面板。本地 `npm test -- h5-experience`、`npm run typecheck`、`npm test`、`npm run build:demo`、`git diff --check` 通过；全量为 34 个测试文件、219 tests，另有 2 个 integration 文件 skipped。记录见 `nnz-mvp-2026-07-08-Step2.43-H5SealInlineConfirm.md`。
