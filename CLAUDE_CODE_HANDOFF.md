...
2026-07-07 Step 2.36: sensitive local release safety 本地已实现；`.gitignore` 显式忽略 SQLite/DB、raw/report/summary/evidence JSON、migration/release/snapshot artifact 目录，新增 `nnz-mvp/.env.example` 空模板和 disposable DB 安全注释；check-ignore 验证不影响 package/tsconfig/Markdown/env template；本地 typecheck、206 tests + 2 skipped、build:demo、git diff --check 通过
```
