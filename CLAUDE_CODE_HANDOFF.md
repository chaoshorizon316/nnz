...
2026-07-07 Step 2.37: H5 graduation export + safety support UX 本地已实现；`public/index.html` 的毕业流先导出用户数据档案再提交毕业，高风险 guard 回复出现时显示现实支持提示和热线拨号入口，状态徽标不再兜底显示内部状态名；新增 `src/h5-experience.test.ts` 固化回归；本地 h5 targeted tests、typecheck、全量 tests、build:demo、git diff --check 通过，尚待下一次合并 push
```
