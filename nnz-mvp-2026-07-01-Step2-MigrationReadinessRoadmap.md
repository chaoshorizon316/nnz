...
Step 2 的 scoped repository 与 snapshot migration 工具链已经完成到 Step 2.47。最新已推送提交是：

```text
f21b392 fix: keep h5 lifecycle panels mutually exclusive
```

Step 2.46 H5 panel mutual exclusion 已完成验证并推送。当前本地新增 Step 2.47 H5 visible mechanism leak guard；它不改变本路线图的外部 release validation 剩余入口。
...
- Step 2.47：H5 visible mechanism leak guard 本地已实现；H5 静态测试扫描用户可见正文和常见可见属性，防止内部机制词、后台审核表达和模型表达进入前台文案，并把安全/付费卡片改成现实支持与数据档案语言。
...
- 当前本地 Step 2.47 是 H5 可见文案机制泄露护栏，不依赖外部 DB/token；推送后，核心上线闸口仍是上面的 release validation suite。
