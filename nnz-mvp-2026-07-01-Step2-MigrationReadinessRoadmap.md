...
Step 2 的 scoped repository 与 snapshot migration 工具链已经完成到 Step 2.23。最新已推送提交是：

```text
3669fd4 fix: harden migration disposable database guardrails
```

截至 2026-07-03，迁移链路还剩 **4 个目标未完全收口**：真实本地 snapshot readiness、一次性 Postgres smoke、Render 角色 token smoke、以及真正的 demo runtime scoped-table adapter。受保护执行入口、readiness/smoke CLI、runtime mode guardrail、migration guardrail hardening、scoped runtime adapter foundation，以及 `/api/me/*` 用户端 InMemory adapter wiring 都已完成本地实现。
...
| 4 | demo runtime scoped-table adapter | Step 2.20/2.21 已加 runtime 与 migration guardrails；Step 2.22 已建立 adapter foundation；Step 2.23 已把 `/api/me/*` auth/persona/chat/Covenant flow 接到 InMemory adapter；Postgres scoped runtime mode、Ops/cleanup/export/delete 后续仍待切换 | `/api/me/*`、chat、ops、cleanup、export/delete 都走 scoped tables，仍保持 `userId + personaId` 强隔离 |
...
4. 最后继续目标 4：在 Step 2.23 的 `/api/me/*` adapter wiring 后，把 `NNZ_RUNTIME_PERSISTENCE_MODE=scoped` 接到 Postgres scoped adapter，再扩展 Ops/cleanup/export/delete。
...
- `NNZ_RUNTIME_PERSISTENCE_MODE=scoped` 不使用 `DATABASE_URL` / `NNZ_POSTGRES_URL`，只允许通过 `NNZ_POSTGRES_SCOPED_RUNTIME_URL` 表达未来 scoped runtime 连接；当前仍应 fail-fast，直到 Postgres scoped runtime cutover 完成。
- 任何执行入口都必须保留 `userId + personaId` 作用域边界，不能引入 persona-only 查询。
- 用户端不可暴露 `SoulVersion`、`SoulSnapshot`、`scope`、`evidence`、`migration` 等后台机制。

## 当前可继续做的本地工作

- 将 `NNZ_RUNTIME_PERSISTENCE_MODE=scoped` 连接到 Step 2.22/2.23 adapter，并补 `/api/me/*` scoped Postgres smoke；默认仍保持 snapshot persistence。
