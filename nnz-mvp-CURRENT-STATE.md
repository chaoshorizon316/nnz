...
2026-07-10 Step 2.71: optional Ops audit retention 本地已实现；新增 `NNZ_OPS_AUDIT_RETENTION_DAYS` / `NNZ_OPS_AUDIT_MAX_EVENTS` 正整数配置，空值默认不裁剪；每次 Ops audit write 后裁剪 snapshot/SQLite 与 scoped Postgres Ops 路径，不输出 audit payload、用户内容、token、DB URL 或来源 IP；补充 `.env.example`、README、handoff、roadmap 和 Step 文档
```

当前代码基线相对远端：

```text
main...origin/main @ c4db306 Summary: feat: add soul ops IP allowlist hardening
当前本地新增 Step 2.71 optional Ops audit retention pending
```
