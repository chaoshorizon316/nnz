...
```text
npm run typecheck
npm test         # 13 files, 87 tests passed
npm run build:demo
```

下一步：优先在 Render 验证可选角色 token（viewer/operator/admin）的云端权限边界；工程侧进入真实 Postgres 集成测试与 snapshot -> tables 迁移设计。token 明文不得写入仓库或文档。
