...
4. 继续补 Ops 生产化护栏：session 策略、正式云 perimeter；如果后续代码再改变 migration/runtime/Ops 边界，复用同一套入口重跑 `release:validation-suite`。

当前不需要每个小步骤都停下来等 push；Step 2.71 Ops audit retention 可作为下一版合并 push。现在 release validation 已过，Render 免费调试环境过期风险已按非生产环境记录；后续正式环境优先做腾讯云方案评估。完整路线图见 `../nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md`。
...
接手时先看 `nnz-mvp-2026-07-10-Step2.69-ReleaseValidationSuitePassed.md`、`nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md` 和本地最新 Step 2.71 记录。下一步不是再配置数据库、再拆 `/demo`、再启用 `/ops`、再加基础 audit log/RBAC、再注入 release 外部输入；而是继续围绕 Ops 生产化护栏、正式环境方案评估、以及必要时的 release suite 复跑做收口。
