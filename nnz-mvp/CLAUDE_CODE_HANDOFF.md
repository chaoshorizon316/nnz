...
Step 2 scoped repository、snapshot migration 工具链、当前前台发布收口、release env-file inputs、focused release stage env-file、Render role-specific Ops token smoke 和真实 release validation suite 均已完成并推送到 `2cbe5b1 docs: record render debug stance and future tencent cloud plan`。2026-07-10 已在 Render 配置 `NNZ_OPS_VIEWER_TOKEN` / `NNZ_OPS_OPERATOR_TOKEN` / `NNZ_OPS_ADMIN_TOKEN`，同值仅存 ignored `.env.release`，并通过非破坏性 `ops:role-smoke -- --env-file .env.release --base-url https://nnz-kego.onrender.com --confirm RUN_OPS_ROLE_TOKEN_SMOKE`。2026-07-10 在用户授权后已补齐真实 snapshot、migration 临时 DB、scoped runtime 临时 DB，并完整通过 `release:validation-suite`：`releasePreflight` / `migrationValidationSuite` / `opsRoleSmoke` / `runtimeSmokeSuite` 全部 passed，evidence 写入 ignored `release-artifacts`。本地 Step 2.70 正在补 Soul Ops optional IP allowlist hardening：`NNZ_OPS_ALLOWED_IPS` 为空时保持调试行为，设置后先于 token 检查保护 `/ops` 与 `/api/ops/*`。
...
4. 继续补 Ops 生产化护栏：session 策略、审计保留策略、正式云 perimeter；如果后续代码再改变 migration/runtime/Ops 边界，复用同一套入口重跑 `release:validation-suite`。

当前不需要每个小步骤都停下来等 push；Step 2.70 Ops IP allowlist hardening 可作为下一版合并 push。现在 release validation 已过，Render 免费调试环境过期风险已按非生产环境记录；后续正式环境优先做腾讯云方案评估。完整路线图见 `../nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md`。
...
接手时先看 `nnz-mvp-2026-07-10-Step2.69-ReleaseValidationSuitePassed.md`、`nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md` 和本地最新 Step 2.70 记录。下一步不是再配置数据库、再拆 `/demo`、再启用 `/ops`、再加基础 audit log/RBAC、再注入 release 外部输入；而是继续围绕 Ops 生产化护栏、正式环境方案评估、以及必要时的 release suite 复跑做收口。
