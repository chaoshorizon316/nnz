...
Step 2 scoped repository、snapshot migration 工具链、当前前台发布收口、release env-file inputs、focused release stage env-file、Render role-specific Ops token smoke 和真实 release validation suite 均已完成。2026-07-10 已在 Render 配置 `NNZ_OPS_VIEWER_TOKEN` / `NNZ_OPS_OPERATOR_TOKEN` / `NNZ_OPS_ADMIN_TOKEN`，同值仅存 ignored `.env.release`，并通过非破坏性 `ops:role-smoke -- --env-file .env.release --base-url https://nnz-kego.onrender.com --confirm RUN_OPS_ROLE_TOKEN_SMOKE`。2026-07-10 在用户授权后已补齐真实 snapshot、migration 临时 DB、scoped runtime 临时 DB，并完整通过 `release:validation-suite`：`releasePreflight` / `migrationValidationSuite` / `opsRoleSmoke` / `runtimeSmokeSuite` 全部 passed，evidence 写入 ignored `release-artifacts`。Step 2.70 Soul Ops optional IP allowlist hardening 已推送到 `c4db306 Summary: feat: add soul ops IP allowlist hardening`。Step 2.71 optional Ops audit retention 已推送到 `f63209a feat: add ops audit retention policy`。Step 2.72 optional short-lived Ops sessions 已推送到 `701266d feat: add short-lived ops sessions`：`NNZ_OPS_SESSION_TTL_MINUTES` 为空时保留 role token 直连，设置后 role token 只用于创建短期 session，`/api/ops/*` 只接受 session token。当前稳定基线已封存到 `../nnz-mvp-2026-07-10-StableBaseline-Step2.72-ExternalAuditArchive.md`，供另一个 AI 做完整度、进度和风险评估。

2026-07-17 用户已定下一阶段路线：先用 H5 / Web App 内测，支持聊天记录文件上传；微信方向不再做缺口评估，直接通过微信机器人接入流程打通当前服务。本地已新增 `src/chat-record-import.ts`、H5 上传 `.txt` / `.json` 聊天记录、`POST /api/me/chat-upload`、H5 微信接入码、`NNZ_WECHAT_BOT_TOKEN`、`POST /api/wechat-bot/link` 和 `POST /api/wechat-bot/message`。机器人消息复用 `sendMessageToUserPersona()`，仍保持 `userId + personaId` scoped runtime。当前机器人 binding 是进程内 Map，只适合内测；正式环境前需要持久化绑定或正式微信 OAuth 身份链路。
...
当前本地检查结论：ignored `.env.release` 已具备 release validation 所需 key：`NNZ_MIGRATION_SNAPSHOT_PATH`、`NNZ_POSTGRES_INTEGRATION_URL`、`NNZ_POSTGRES_SCOPED_RUNTIME_URL`、`NNZ_OPS_VIEWER_TOKEN`、`NNZ_OPS_OPERATOR_TOKEN`、`NNZ_OPS_ADMIN_TOKEN`。生产 DB URL 只被临时用于导出 snapshot，已从 `.env.release` 移除。raw snapshot、raw suite snapshot、migration report/summary、evidence 均在 ignored `release-artifacts`，不能提交、截图或粘贴。

1. Render 暂按免费调试环境处理；Dashboard 显示 `nnz-mvp-postgres` 会在 2026-07-11 过期并删除，但该环境不作为正式生产持久化承诺。
2. 保留 ignored `.env.release` 和 `release-artifacts` 到本轮验收完成；不要提交、截图或粘贴 secret/raw snapshot。
3. 正式环境迁移/上线前，优先评估腾讯云方案（应用托管、PostgreSQL、对象存储/备份、域名/证书、日志监控、安全组与成本），由用户评估后执行。
4. 本地 Ops 生产化护栏已收口到 role token、IP allowlist、audit retention、short-lived sessions；如果后续代码再改变 migration/runtime/Ops 边界，复用同一套入口重跑 `release:validation-suite`。

当前不需要每个小步骤都停下来等 push；Step 2.72 已推送，本地 stable baseline archive、H5 chat upload 与 WeChat bot bridge 可作为下一版合并 push。现在 release validation 已过，Render 免费调试环境过期风险已按非生产环境记录；后续先做 H5/Web App 内测与机器人桥接实测，再做腾讯云正式环境评估。完整路线图见 `../nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md`。
...
接手时先看 `nnz-mvp-2026-07-10-StableBaseline-Step2.72-ExternalAuditArchive.md`、`nnz-mvp-2026-07-10-Step2.69-ReleaseValidationSuitePassed.md`、`nnz-mvp-2026-07-01-Step2-MigrationReadinessRoadmap.md` 和本地最新 Step 2.72 记录。下一步不是再配置数据库、再拆 `/demo`、再启用 `/ops`、再加基础 audit log/RBAC、再注入 release 外部输入；而是进入正式环境方案评估、正式迁移/切换，或在代码边界变化后复跑 release suite。
