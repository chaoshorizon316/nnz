...
With Step 2.24-2.26, the guarded scoped mode initializes the scoped Postgres schema and routes `/api/me/*` runtime calls through the Postgres scoped runtime adapter when `NNZ_POSTGRES_SCOPED_RUNTIME_URL` is present. Scoped mode also has a first Ops cleanup/audit cutover: cleanup dry-run/confirm and Ops audit query/write can use scoped Postgres tables. Validate scoped tables with `migration:smoke` and scoped runtime behavior with `runtime:smoke` before any production switch; default runtime remains `snapshot`, and full Ops overview plus user export/delete are not yet cut over to scoped tables.
