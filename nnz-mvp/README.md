...
Next engineering steps: push the local Step 2.38 UX batch when ready, then inject external inputs, run `release:validation-suite -- --evidence-out <sanitized-evidence-json-path>`, and only split into individual stage commands when a failure needs focused diagnosis.
