...
With Step 2.24-2.35, the guarded scoped mode initializes the scoped Postgres schema and routes `/api/me/*` runtime calls through the Postgres scoped runtime adapter when `NNZ_POSTGRES_SCOPED_RUNTIME_URL` is present. Scoped mode also has Ops cleanup/audit/overview cutover slices and user data sovereignty endpoints: cleanup dry-run/confirm, Ops audit query/write, `/api/ops/overview` user/persona/maturity aggregation, and `/api/me/export` / `/api/me/delete` can use scoped Postgres tables. Use `release:validation-suite` to run release preflight, migration validation, role-specific Ops token smoke, and scoped runtime smoke suite in order before any production switch; optional `--evidence-out` writes a sanitized launch evidence JSON. Default runtime remains `snapshot`.
...
POST /api/me/persona
POST /api/me/memory
POST /api/me/chat-upload
POST /api/me/wechat-bot-link
...
Chat record upload for H5 internal testing uses `POST /api/me/chat-upload`. Recommended user format is UTF-8 `.txt`, one message per line, `speaker: content` or `说话人：内容`. Structured `.json` is also accepted when the root object has a `messages` array, or the file itself is a message array. Uploaded records become `CHAT_EXCERPT` memories with source `UPLOAD`; the response returns only counts and metadata, not the imported excerpt.

The internal WeChat bot bridge is token-protected with `NNZ_WECHAT_BOT_TOKEN`. H5 users generate a short link code with `POST /api/me/wechat-bot-link`; the bot service then calls `POST /api/wechat-bot/link` with `externalUserId` and `linkCode`, and sends user messages through `POST /api/wechat-bot/message`. Bot responses use the same scoped runtime as `/api/me/chat` and do not return internal `userId` or `personaId`. Current bot bindings are in-memory and intended for internal testing, not final WeChat OAuth production identity.
...
Current verified suite on 2026-07-07: 34 test files / 215 tests plus two skipped opt-in Postgres integration tests across domain scope, scoped repositories, scoped runtime adapter/persistence, scoped user export/delete, scoped Ops cleanup/audit/overview, Soul Ops cleanup/overview/audit query/RBAC, runtime persistence config guardrails, SQLite/Postgres snapshot persistence, Postgres scoped repository, snapshot export, snapshot migration planner/row builder/executor/readiness/smoke/validation-suite CLI guardrails, Soul Ops role token smoke CLI guardrails, release preflight/validation-suite/evidence CLI guardrails, scoped runtime smoke CLI guardrails, scoped runtime HTTP smoke CLI guardrails, scoped runtime smoke suite guardrails, H5 graduation/export/safety support, H5 onboarding consent, H5 account deletion confirmation, H5 memory append, H5 graduation inline confirmation, auth, runtime, LLM prompt contract, safety guard, LLM adapter, and extraction orchestrator. Local `/api/me/*` smoke also passes for register, persona creation, memory append, chat, history, seal, activate node, and complete node.
...
This command is the preferred target-1/2 validation entry once a real local snapshot and a disposable Postgres database are available. It first runs offline `migration:readiness`; only if readiness exits cleanly does it run `migration:smoke` against `NNZ_POSTGRES_INTEGRATION_URL`. Optional `--env-file` explicitly loads ignored local env files for focused diagnosis. It refuses `DATABASE_URL`, rejects disposable URL alias conflicts, writes the same raw snapshot/report/summary outputs as readiness, and does not print env file paths, database URLs, memory/chat text, credential hashes, raw snapshot data, row payloads, child command output, or raw error details.
...
This command is the preferred target-4 validation entry once a disposable scoped runtime database is available. It first runs `runtime:smoke`, then runs `npm run build:demo`, then runs `runtime:http-smoke` with the same guarded scoped runtime env. Optional `--env-file` explicitly loads ignored local env files for focused diagnosis. It accepts `--skip-build` only when the demo server build output is already current. It refuses `DATABASE_URL`, rejects scoped runtime URL alias conflicts, and does not print env file paths, database URLs, tokens, email/password values, memory/chat text, credential hashes, row payloads, child process output, server logs, or raw error details.
...
This command is the preferred target-3 verification entry once Render has role-specific tokens and the same values are available in the local shell or ignored env file as `NNZ_OPS_VIEWER_TOKEN`, `NNZ_OPS_OPERATOR_TOKEN`, and `NNZ_OPS_ADMIN_TOKEN`. It verifies missing/invalid token rejection, viewer overview/audit read, viewer cleanup denial, operator cleanup dry-run, operator delete denial, admin cleanup dry-run, and the admin delete confirmation boundary. Default mode is non-destructive; a real confirmed cleanup check requires `--include-delete --delete-confirm RUN_OPS_ROLE_TOKEN_DELETE_SMOKE`. The command does not print env file paths, token values, response payloads, user content, cleanup receipts, server logs, or raw network details.
...
This command checks whether the three remaining external validation groups have their local prerequisites: a readable snapshot/SQLite input, disposable migration database env, role-specific Ops token envs, and disposable scoped runtime database env. Optional `--env-file` explicitly loads local-only env files such as ignored `.env.release`; existing shell env values win over file values. It does not read snapshot contents, connect to databases, send network requests, or print env file paths, snapshot paths, database URLs, token values, user content, cleanup receipts, server logs, or raw network details.
...
npm run release:validation-suite -- --env-file .env.release --from-sqlite-env NNZ_DB_PATH --snapshot-out <raw-snapshot-json-path> --report-out <sanitized-report-json-path> --summary-out <sanitized-summary-json-path> --evidence-out <sanitized-evidence-json-path> --confirm RUN_NNZ_RELEASE_VALIDATION_SUITE
```

This is the preferred one-command release validation entry once the external inputs are available. It runs `release:preflight`, `migration:validation-suite`, default non-destructive `ops:role-smoke`, and `runtime:smoke-suite` in order. Optional `--env-file` explicitly loads local-only env files; `--from-json-env` and `--from-sqlite-env` resolve the snapshot input path from an env key without printing that path. Optional `--evidence-out` writes a sanitized release evidence JSON with stage status, env key names, local artifact categories, and redaction notes. It does not run confirmed Ops cleanup deletion and does not print env file paths, database URLs, token values, snapshot paths, snapshot contents, user content, child command output, server logs, raw error details, or evidence output paths.
...
Step 2.3 cloud status on 2026-06-17: `/api/ops/audit-events` and the `/ops` Audit tab are implemented and pushed. GitHub Actions run `27677337466` passed. Render `/healthz` reports Postgres persistence, `/ops` returns 200 and includes the Audit tab, `/api/ops/audit-events` returns 401 without a token and 403 with a wrong token. Step 2.32 adds `ops:role-smoke` for the remaining cloud role-specific token verification after Render has `NNZ_OPS_VIEWER_TOKEN`, `NNZ_OPS_OPERATOR_TOKEN`, and `NNZ_OPS_ADMIN_TOKEN` configured. Step 2.33 adds `release:preflight` to summarize which external inputs are present before running any network or database smoke. Step 2.34 adds `release:validation-suite` to run the remaining validations as one protected sequence. Step 2.35 adds optional sanitized release evidence output for handoff and launch audit. Step 2.36 adds `.gitignore` protection for sensitive local release/migration artifacts and a safe `.env.example` template. Step 2.66 adds explicit `--env-file`, `--from-json-env`, and `--from-sqlite-env` release inputs for ignored local env files. Step 2.67 adds focused stage `--env-file` parity for migration/runtime/Ops diagnosis. Step 2.37-2.47 complete the latest H5 user journey hardening through graduation/export, consent, deletion, memory append, lifecycle confirmations, panel mutual exclusion, and visible mechanism leak guards.

Step 2 cloud role token status on 2026-07-10: Render has `NNZ_OPS_VIEWER_TOKEN`, `NNZ_OPS_OPERATOR_TOKEN`, and `NNZ_OPS_ADMIN_TOKEN` configured, with the same values stored only in ignored local `.env.release`. The non-destructive role smoke passed with `npm run ops:role-smoke -- --env-file .env.release --base-url https://nnz-kego.onrender.com --confirm RUN_OPS_ROLE_TOKEN_SMOKE`, verifying missing/invalid token rejection, viewer read-only access, operator dry-run, admin dry-run, admin delete confirmation boundary, and audit query read. Token values were not printed, committed, or documented.

Optional Soul Ops IP allowlist: set `NNZ_OPS_ALLOWED_IPS` to comma-separated exact IP addresses or IPv4 CIDR ranges to protect both `/ops` and `/api/ops/*` before token checks. Leave it blank for local/debug environments. In proxied deployments, the first `x-forwarded-for` address is used; access-denied audit metadata records only that an IP was present and that the allowlist was enabled, not the raw IP value.

Optional Soul Ops audit retention: set `NNZ_OPS_AUDIT_RETENTION_DAYS` and/or `NNZ_OPS_AUDIT_MAX_EVENTS` to positive integers to prune Ops audit events after each audit write. Leave both blank for local/debug environments. Retention runs in both snapshot/SQLite paths and scoped Postgres Ops mode without printing audit payloads, raw user content, token values, database URLs, or source IPs.

Step 2 release validation status on 2026-07-10: with explicit user authorization, the current Render snapshot persistence was exported to ignored local `release-artifacts`, two isolated temporary Postgres databases were created inside the existing Render Postgres instance for migration/runtime validation, and `release:validation-suite` passed all stages with `--env-file .env.release`, `--from-json-env NNZ_MIGRATION_SNAPSHOT_PATH`, and `--evidence-out`. The suite reported `releasePreflight`, `migrationValidationSuite`, `opsRoleSmoke`, and `runtimeSmokeSuite` as passed. Secret values, DB URLs, raw snapshot JSON, user content, and raw child output were not printed or documented. Render remains a free debugging environment for now; formal production migration should be evaluated separately, with Tencent Cloud as the preferred candidate for later review.
...
Next engineering steps: run H5/Web App internal testing with `.txt` / `.json` chat record upload, wire an actual WeChat bot process to `/api/wechat-bot/*`, then evaluate Tencent Cloud formal deployment before the formal environment migration/switch. If later code changes touch migration/runtime/Ops boundaries, rerun `release:validation-suite` with the ignored `.env.release` inputs while they remain available.
